...
export default function ProductDetail({ product, onBack, onNavigate, onBuyNow, onProductClick }: ProductDetailProps) {
  ...
  const handleBuyNow = () => {
    ...
    onBuyNow('', [{ id: product.id, name: product.name, quantity: 1, price: displayPrice }], displayPrice);
  };

  const [hasApplePay, setHasApplePay] = useState(false);
  useEffect(() => {
    setHasApplePay('ApplePaySession' in window);
  }, []);
              ...
              <div className="space-y-3">
                ...
                {hasApplePay && (
                  <button
                    type="button"
                    onClick={handleBuyNow}
                    className="w-full h-[52px] bg-black text-white rounded-md flex items-center justify-center gap-2 text-[15px] font-medium hover:opacity-90 transition-opacity"
          ...
          >
            ...
            {onBuyNow && hasApplePay && (
              <button
                type="button"
                onClick={handleBuyNow}
                className="flex-1 py-4 bg-black text-white flex items-center justify-center gap-1.5 text-xs font-bold uppercase tracking-[0.2em]"
                style={{ fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif' }}
              >
